...
background = cv2.imread('foto_capturada1.jpg')  # Reemplaza con tu imagen de fondo
background = cv2.resize(background, (736, 416))  # Ajusta el tamaño según tu cámara
...
while True:
    ...
    resultados = model.predict(frame, imgsz=736, conf=0.78 )
    ...
    if masks is None:
        poligono=np.zeros((736, 416)).astype("uint8")
    else:

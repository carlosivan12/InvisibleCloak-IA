...
""" result=resultados[0]
    mascaras = result.masks

    mascara1 = mascaras[0]
    dato1 = mascara1.data[0].cpu().numpy() #cambiar de arreglo tensor a arreglo numpy
    poligono = mascara1.xy[0]
    dato1im=np.uint8(dato1)
    enmascarada = cv2.bitwise_and(background,background, mask=dato1im)
    cv2.imshow('Invisibility Cloak', cv2.cvtColor(enmascarada, cv2.COLOR_BGR2RGB)) """